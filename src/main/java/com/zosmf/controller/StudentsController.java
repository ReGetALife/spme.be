...
public class StudentsController {
    ...
    public List<Map<String, Object>> getDraft(@RequestBody Map<String, String> req, HttpSession session){
        ...
        if (ZOSMF_JSESSIONID == null || ZOSMF_LtpaToken2 == null || ZOSMF_Address == null || ZOSMF_Account == null) {
            ...
            String sql = "select answer from report where is_draft = 1 and uid=? and lab=? and step=? and lower_lab=? and question_id=?";
    ...
    public List<Map<String, Object>> subAnswer(@RequestBody Map<String, String> req, HttpSession session) {
        ...
        if (ZOSMF_JSESSIONID == null || ZOSMF_LtpaToken2 == null || ZOSMF_Address == null || ZOSMF_Account == null) {
            ...
            String lower_lab = req.get("lower_lab");
            String question_id = req.get("question_id");
            String is_draft = req.get("is_draft");
            ...
            if (result_list.size() == 0) {
                ...
                jdbcTemplate.update(sql_insert, uid, lab, step, lower_lab, question_id, is_draft, answer);
            } else {
                String sql_update = "update report set answer=? ,is_draft=? where uid=? and lab=? and step=? and lower_lab=? and question_id=?;";
                jdbcTemplate.update(sql_update, answer, is_draft, uid, lab, step, lower_lab, question_id);
            }
