...
public class DemoController {
    ...
    @CrossOrigin(origins = "*", allowCredentials = "true")
    @RequestMapping(value = "/racf/users", method = RequestMethod.POST)
    public ResponseEntity<String> addUser(@RequestBody Map<String, String> user, HttpSession session) {
//获取session数据
        Object ZOSMF_JSESSIONID = session.getAttribute("ZOSMF_JSESSIONID");
        Object ZOSMF_LtpaToken2 = session.getAttribute("ZOSMF_LtpaToken2");
        Object ZOSMF_Address = session.getAttribute("ZOSMF_Address");
        if (ZOSMF_JSESSIONID == null || ZOSMF_LtpaToken2 == null || ZOSMF_Address == null) {
            ...
            headers.add("Cookie", ZOSMF_JSESSIONID.toString() + ";" + ZOSMF_LtpaToken2);
            //构建racf命令
            StringBuilder command = new StringBuilder("AU ");
            if (user.get("userid") != null && !user.get("userid").equals("")) {
                command.append(user.get("userid")).append(" ");
            }
            if (user.get("owner") != null && !user.get("owner").equals("")) {
                command.append("OWNER(").append(user.get("owner")).append(") ");
            }
            if (user.get("dfltgrp") != null && !user.get("dfltgrp").equals("")) {
                command.append("DFLTGRP(").append(user.get("dfltgrp")).append(") ");
            }
            if (user.get("authority") != null && !user.get("authority").equals("")) {
                command.append("AUTHORITY(").append(user.get("authority")).append(") ");
            }
            if (user.get("name") != null && !user.get("name").equals("")) {
                command.append("NAME(").append(user.get("name")).append(") ");
            }
            if (user.get("password") != null && !user.get("password").equals("")) {
                command.append("PASSWORD(").append(user.get("password")).append(") ");
            }
